...
namespace GHIElectronics.Endpoint.UI.Media {
    ...
    public class DrawingContext : DispatcherObject, IDisposable {
        ...
        public void DrawPolygon(Brush brush, Pen pen, int[] pts) {
            ...
            if (nPts > 2) {
                this.DrawLine(pen, pts[nPts * 2 - 2], pts[nPts * 2 - 1], pts[0], pts[1]);
            }
        }
        ...
        public void DrawText(string text, GHIElectronics.Endpoint.Drawing.Font font, Color color, int x, int y) {
            this.VerifyAccess();

            this._bitmap.DrawText(text, font, color, this._x + x, this._y + y);
        }

        public bool DrawText(ref string text, GHIElectronics.Endpoint.Drawing.Font font, Color color, int x, int y, int width, int height,
