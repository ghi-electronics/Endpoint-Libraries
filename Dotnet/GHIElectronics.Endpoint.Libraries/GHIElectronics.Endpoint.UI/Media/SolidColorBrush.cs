        ...
        internal override void RenderPolygon(Bitmap bmp, Pen outline, int[] pts) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (lines[i].y2 < lines[i].y1) {
                    this.Swap(ref lines[i].y2, ref lines[i].y1);
                    this.Swap(ref lines[i].x2, ref lines[i].x1);
                }

                /// Calculate slopes and increments.
                lines[i].dx = this.Abs(lines[i].x2 - lines[i].x1);
                lines[i].dy = this.Abs(lines[i].y2 - lines[i].y1);
