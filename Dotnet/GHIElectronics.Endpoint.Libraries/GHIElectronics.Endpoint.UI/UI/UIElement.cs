...
namespace GHIElectronics.Endpoint.UI {
    public abstract class UIElement : DispatcherObject {
                ...
                case 2: // (byte)EventCategory.Gesture:
                    {
                        ...
                        if (ge.Gesture == TouchGesture.Begin) {
                            this.OnTouchGestureStarted(ge);
                        }
                        else if (ge.Gesture == TouchGesture.End) {
                            this.OnTouchGestureEnded(ge);
                        }
                        else {
                            this.OnTouchGestureChanged(ge);
                        }
        ...
        public int Height {
            get {
                if (this.IsHeightSet(out var height)) {
                    return height;
                }
                else {
        ...
        public int Width {
            get {
                if (this.IsWidthSet(out var width)) {
                    return width;
                }
                else {
        ...
        public void Measure(int availableWidth, int availableHeight) {
            ...
            try {
                ...
                if (this._requestedSize != null) {
                    ...
                    this.MeasureOverride(frameworkAvailableWidth, frameworkAvailableHeight, out desiredWidth, out desiredHeight);
        ...
        public void Arrange(int finalRectX, int finalRectY, int finalRectWidth, int finalRectHeight) {
            ...
            if (clientWidth != arrangeWidth || clientHeight != arrangeHeight) {
                this.ComputeAlignmentOffset(clientWidth, clientHeight, arrangeWidth, arrangeHeight, out offsetX, out offsetY);
#if TINYCLR_DEBUG_LAYOUT
