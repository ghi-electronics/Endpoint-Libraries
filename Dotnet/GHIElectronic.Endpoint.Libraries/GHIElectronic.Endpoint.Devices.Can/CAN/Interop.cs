...
namespace GHIElectronic.Endpoint.Devices.Can {
    internal class Interop
    {
        ...
        public static unsafe void Write(SafeHandle handle, byte* buffer, int length)
        {
            ...
            {
                var bytesWritten = Interop.SocketWrite((int)handle.DangerousGetHandle(), buffer, length);
                if (bytesWritten < 0)
                {
        ...
        {
            var bytesRead = Interop.SocketRead((int)handle.DangerousGetHandle(), buffer, length);
            if (bytesRead < 0)
            {
        ...
        public static IntPtr CreateCanRawSocket(string networkInterface)
        {
            ...
            var socket = CreateNativeSocket(PF_CAN, SOCK_RAW, CanProtocol.CAN_RAW);
            ...
            where T : struct
        {
            ...
            var buf = MemoryMarshal.AsBytes(data);
        ...
        {
            var idx = GetInterfaceIndex(fd, interfaceName);
            var addr = new CanSocketAddress {
                can_family = PF_CAN,
                can_ifindex = idx
            };
        ...
        private static unsafe int GetInterfaceIndex(int fd, string name)
        {
            ...
            {
                var written = Encoding.ASCII.GetBytes(inIntefaceName, name.Length, ifr.ifr_name, MaxLen);
                ifr.ifr_name[written] = 0;
            }

            var ret = Ioctl3(fd, SIOCGIFINDEX, ref ifr);
            if (ret == -1)
            {
        ...
        internal unsafe struct Ifreq
        {
            ...
            private fixed byte padding[IFNAMSIZ - sizeof(int)];
